...
In RStudio first we need to install the plotly package. Either select the Packages tab in RStudio and type plotly and install, or type the following in the console and Enter.  
...
```{r}
Sys.setenv("poldham" = "me", "7caig7wrwb" = "mykey")
```


Next we will use a quick plot or `qplot` from `ggplot2`. First we will load the pizza patents by country and year dataset from the Github repository using `readr`. First you may need to install the `readr` package and/or load the library. 
...
Now read in the dataset from the Github respository (if you have it downloaded already you could use Tools and Import Dataset to import the file).

```{r eval=FALSE}
pcy <- read_csv("https://github.com/poldham/opensource-patent-analytics/raw/master/2_datasets/pizza_medium_clean/pcy.csv")
```

The pcy object contains WIPO Patentscop data on patent documents containing the term pizza by year and country (pcy = pizza country year). 

Because patent data generally contains a data cliff for more recent years we will filter out recent years using the dplyr package. 

```{r}
library(dplyr)
pcy <- filter(pcy, pubyear)
```


To create the plot in plotly we can use `plot_ly()`. 

```{r}
s <- plot_ly(pcy, x = pubyear, y = n, color = pubcountry, type = "scatter")
s1 <- layout(s, title = "Pizza Patent trends")

plotly_POST(s)
```

If we do this then plotly will record a warning that the colour palette has a maximum number of 8 colors. This means that some countries will share a colour. There are 16 countries in the pizza dataset and so we need to change the colour palette to reflect this.

To do this it is helpful to note that on installation plotly installs and then calls the RColorBrewer package (it will display in the Packages list). To see the colour palettes we first tick RColorBrewer in packages (or library(RColorBrewer)) to load it. 

```{r}
pizza_plot <- ggplotly(pizza)
```
