---
title: 'Graphing Patent Data in R: Part 1'
output: word_document
published: yes
layout: post
---
```{r global_options, include=FALSE}
knitr::opts_chunk$set(cache=TRUE, tidy=TRUE, fig.width=12, fig.height=8, fig.align = "center",
                      echo=TRUE, warning=FALSE, message=FALSE)
```
This is the first part of a two part chapter on using R and the `ggplot2` package to visualise patent data. In a previous chapter we looked at visualising pizza related patent activity in Tableau Public. In this chapter we look at how to plot our `pizza` [dataset]() using the `ggplot2` package in RStudio. You do not need to know anything about R to follow this chapter. You will however need to install  [RStudio Desktop](http://www.rstudio.com/products/rstudio/) for your operating system (see below).
...
In this chapter we will start with qplot and increasingly merge into developing plots by adding layers in what could be called a ggplot kind of way. We will develop that further in Part 2.  

##Getting Started with R

This chapter assumes that you are new to using R. You do not need any knowledge of programming in R to follow this chapter. While you don't need to know anything about R to follow the chapter, you may find it helpful to know that:
...
2. R works with a combination of base R as the core language and additional packages (libraries) for performing tasks such as importing files, manipulating files and graphics. There are around 6,819 packages and they are open source (mainly it seems under the MIT licence). If you can think of it there is probably a package that meets (or almost meets) your analysis needs.
...
5. If you want to learn more, or get stuck, there are a large number of resources and free courses out there and RStudio lists some of the main resources on their website [here](http://www.rstudio.com/resources/training/online-learning/). With R you are never alone. 
...
In this chapter we will use four packages:
...
In this chapter we will focus on the `pizza` table as the core set. However, you may want to experiment with other sets. 

##Reading in the Data

We will use the `readr` package to rapidly read in the pizza set to R (for other options see the in depth chapters on reading in [.csv]() and [Excel]() files and the recent Getting your Data into R RStudio [webinar](http://www.rstudio.com/resources/webinars/)). `readr` is nice and easy to use and creates a data table that we can easily view. 

```{r read_csv, tidy=TRUE}
...
See a separate table in a new tab. This is useful if you want to get a sense of the data or look for column numbers. Note the capital V in View. 
...
If we now use `View(pizza)` we will see a new column called `n` with a value of 1 for each entry at the end of the data frame. 
...
To make life even easier for ourselves we can use function `count()` from `dplyr` to group the data onto counts by different variables for graphing (see also `summarise()`, `group_by()` and `tally()`). Note that we could defer counting until later, however, this is a good opportunity to learn more about `dplyr`. 

Let's go ahead and construct some counts using `p1`. At the same time we will use quick plot (`qplot`) for some exploratory plotting of the results. In the course of this R will show error warnings in red for missing values. We will be ignoring the warning because they are often R telling us things it thinks we need to know.
...
In this code chunk we have asked count to produce a count of the data using the publication with n as the weight (wt) to count by. When we view `pt` (also try using `View(pt)`, noting the capital V, or clicking `pt` in the Environment pane) we will see that R has dropped the country columns to present us with an overall total by year in `n`. We now have a general overview of the data for graphing.
...
<!---![_config.yml](images/fggplot1/fig1_ggplot1-1.png)--->
...
```{r fig2_ggplot1, warning=FALSE, }
qplot(x = pubyear, y = n, data = pt, geom = "point")
```

<!---![_config.yml](images/fggplot1/fig2_ggplot1-1.png)--->

What we now see is that we have a single data point in 1940, zero entries up until the late 1960s and that we have a data cliff between around 2012 and 2015. We will come back to this below.

If we wanted to create a quick bar chart of the same data using `qplot` we would use the following. Note that we have specified the x axis and the data, the weight as n and then `geom = "bar"` to generate the graph. 

```{r fig3_ggplot1}
qplot(x = pubyear, data = pt, weight = n, geom = "bar")
```

<!---![_config.yml](images/fggplot1/fig3_ggplot1-1.png)--->
...
To quickly plot the data we will use the `pubcode` (country code) as the value for the x axis to avoid squished country names. We will also add an argument for the colour by specifying color = pubcode.

```{r fig4_ggplot1, warning=FALSE, }
qplot(pubcode, n, data = pc, colour = pubcode)
```

<!---![_config.yml](images/fggplot1/fig4_ggplot1-1.png)--->
...
We now want to gain a quick view of trends by country, we will add colour (as colour = pubcountry) to help distinguish between countries in a line plot. Note that in `ggplot2` `colour` refers to the colour of lines or outlines while `fill` refers to the colour for an object such as a bar. 

```{r fig6_ggplot1, warning=FALSE, }
qplot(pubyear, n, data = pcy, geom = "line", colour = pubcountry)
```

<!---![_config.yml](images/fggplot1/fig6_ggplot1-1.png)--->
...
The key issue here is arriving at a more accurate view. In particular, we do not want to mislead readers into thinking that there has been a massive collapse in the use of the term pizza in patents in recent years. That would be terrible. To do that with `qplot` we can limit the x axis to take out the sparse data and to pull back a sensible distance from the data cliff. In terms of the patent data cliff two years may be sufficient although three years is more sensible. 
...
```{r fig7_ggplot1, warning=FALSE, }
qplot(pubyear, n, data = pcy, geom = "line", 
      colour = pubcountry, 
      xlim = c(1970, 2012))
```

<!---![_config.yml](images/fggplot1/fig7_ggplot1-1.png)--->

R will throw a warning about removing rows that we will ignore. Note that what we have done here is limited the data displayed on the x axis using `xlim`. We have not removed the data from our `pcy` table. We will demonstrate how to edit data out using `filter` in part 2. 
...
<!---![_config.yml](images/fggplot1/fig8_ggplot1-1.png)--->

#Dealing with Legends

We could now do with addressing the legend that reads pubcountry. We will do this by adding `labs()` to the code.

```{r fig9_ggplot1, warning=FALSE, }
qplot(pubyear, n, data = pcy, geom = "line", colour = pubcountry, 
      xlim = c(1970, 2012), 
      xlab = "Publication Year", 
      ylab = "Publication Count", 
...
<!---![_config.yml](images/fggplot1/fig9_ggplot1-1.png)--->
...
<!---![_config.yml](images/fggplot1/fig10_ggplot1-1.png)--->
...
<!---![_config.yml](images/fggplot1/fig11_ggplot1-1.png)--->
...
<!---![_config.yml](images/fggplot1/fig12_ggplot1-1.png)--->

As this demonstrates, there are a variety of options that are available for annotating plots... although as the present example reveals they need to be handled with caution. However, we now have a more or less reasonable line graph. However, to finish off this chapter maybe we should try different visual themes. Let's do that. 
...
<!---![_config.yml](images/fggplot1/fig13_ggplot1-1.png)--->
...
```{r fig14_ggplot1, warning=FALSE, }
line + theme_bw()
```

<!---![_config.yml](images/fggplot1/fig14_ggplot1-1.png)--->
...
```{r fig15_ggplot1, warning=FALSE, }
line + theme_economist() + theme(legend.text=element_text(size=10))
```

<!---![_config.yml](images/fggplot1/fig15_ggplot1-1.png)--->

Here we might want to make some adjustments to the legend size (see above) but it's not bad. 

In a previous chapter we used Tableau Public. We can more or less reproduce a Tableau plot as follows.  

```{r fig16_ggplot1, warning=FALSE, }
line  + theme_igray() + scale_colour_tableau("tableau20")
```

<!---![_config.yml](images/fggplot1/fig16_ggplot1-1.png)--->

In this case we have added `theme_igray()` to grey the background rather than the plot area. We have also specified `scale_colour_tableau`. Note that in some cases a range of colour palettes are available. The default in the Tableau theme is the "tableau10"" scheme with ten colours. This means, in this case, if we do not specify "tableau20" we will not see all of the lines because we don't have enough colours. 
...
A reader familiar with R and `ggplot2` will have noticed that as we added elements to the `qplot` graphics using `+` we are effectively starting to use the `ggplot2` functions to control what we see on a graphic. More accurately, we are increasingly adding layers to the graphic with tighter and tighter control over what we are seeing. That is, we are moving away from using `qplot` as an exploratory graphics tool towards the kind of controls we might use for a publication quality graphic (always bearing in mind we are graphing patent documents mentioning pizza). 

Bear in mind when using R in general that there is normally more than one way to get where you want to go. Thus, there will be faster ways to get to the Tableau plot we have just produced. The `plotly` package discussed elsewhere in this Manual is a good example of an alternative approach. The way we have described above is one way, not necessarily the best way. What is important however, is that we can clearly see the steps involved in building up a graphic. 
...
We have covered saving a `.csv` file in a detailed chapter. If we wanted to save one of our new tables one of the fastest ways to do that is using the `readr` `write_csv()` function. We go into the options for saving data in the chapter Importing CSV files into R and a second chapter on Importing Excel files in R. But, as the code below shows, it is very easy to write a file in R. You may want to use `getwd()` to get the working directory first so that you will know where the file will be stored.

```{r }
write_csv(pcy, "pcy.csv")
```

We can save the current plot easily in a range of formats using the `ggsave` function. We can also save plots from the Plot tab next to the Files tab in a variety of formats. To save the last plot we can simply use:
...
In this chapter we have shown how to get started with R using patent data by focusing on graphing data using the `ggplot2` package and the `qplot` function for our pizza patents. We started this chapter by looking at ways to easily:
