---
title: "Creating a Patent Infographic with R"
author: "Paul Oldham"
...
```{r read}
pizza <- read_csv("https://github.com/wipo-analytics/opensource-patent-analytics/raw/master/2_datasets/pizza_medium_clean/pizza.csv")
```
...
`readr` and `readxl` (for Excel files) are quite new. For more complex data see the Manual online articles on reading csv files in R and read excel files in R packages for importing Excel.
...
For more details on graphing in R see the online qplot and gglot2 articles of the Manual. <!---update file names--->
...
applicant_count <- select(applicants, applicants, publication_number) %>%
  ...
  arrange(desc(n)) %>%
  filter(n >= 1)  %>% #applicant records filter
  print() 
```
...
Note that the correct result for the period 1990 to 2012 for Google is 123 records from 191 records across the whole pizza dataset. The correct result will be achieved only where you use the filtered, separated and trimmed data we created in applicants.  
...
Our final infographic should look something like this online version^[https://infogr.am/trends_in_something].
